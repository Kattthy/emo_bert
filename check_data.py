...
def vision_process(paths):
    ...
    if active_spk_id is None:
        ...
        face_paths = select_faces(_face_dir, active_spk_id) #在faces/....../xx_aligned目录下，找出所有active_spk_id对应的人的脸的图像，并将这些图像的目录按该目录下的图像序号排好序。返回指定人的有序的脸部图像目录列表
        if len(face_paths) < 0.4 * len(glob.glob(os.path.join(_frame_dir, '*.jpg'))):
            fail_flag = True
...
if __name__ == '__main__':
    ...
    print()
    all_positive_clips = []
    with multiprocessing.Manager() as MG: #使用multiprocessing.Manager()实现进程间共享数据
        ...
        all_movies = glob.glob('/data1/hzp/emo_bert/resources/raw_movies/*.mkv')
        all_movies += glob.glob('/data1/hzp/emo_bert/resources/raw_movies/*.mp4')
        raw_data_root = '/data1/hzp/emo_bert/resources/raw_movies'#

        for i, movie in enumerate(all_movies):
            print('[Main]: Processing', movie)
            basename = get_basename(movie) #获取电影文件名（去除扩展名）
            # if basename in ['No0001.The.Shawshank.Redemption', 'No0002.The.Godfather',
            #     'No0003.Inception', 'No0004.Pulp.Fiction', 'No0006.12.Angry.Men', 'No0007.Schindler.List']:
            #     continue
        
            count = MG.dict({ #Manager类型对象，控制了一个server进程，且该对象可被其它进程访问
                'no_sentence': [],
                'too_short': [],
                'is_not_en': [],
                'no_active_spk': [],
            ...
            is_long_enough = list(tqdm(pool.imap(filter_fun, transcript_info), total=len(transcript_info))) #过滤掉长度小于门限值的字幕项
