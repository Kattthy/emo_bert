'''
v6出来的样例太少了，尤其是情感类
这里把视觉模态窗口的策略再用回来，但文本模态还是严格限制
另外，视觉模态对neutral的限制更严，只有一段中所有帧都是neutral，才将该段作为neutral类的有效段
'''
...
class AnnotateEmotion(BaseWorker):
    ...
    '''
    def __init__(self, source_root, audio_root, save_root, logger=None):
        '''
    ...
    def num2faceID(self, face_numID, active_spk_id):
        return 'frame_det_' + str(active_spk_id).zfill(2) + '_' + str(face_numID).zfill(6)
    ...
    def cut(self, seg):
        ...
        '''
        assert len(seg)>=20
        cut_len = math.ceil((len(seg) - 19) / 2) #两边各自需要减去的长度
        new_frames = seg.frames[cut_len: -cut_len]
        return EmotionSegment(new_frames, seg.emotion)
    ...
    def __call__(self, valid_item):
        ...
        success = False
        active_spk_id = None
        speaker_info_file = os.path.join(face_dir, 'has_active_spk.txt')
        if os.path.exists(speaker_info_file):
            with open(speaker_info_file, 'r') as f:
                active_spk_id = f.readline().strip().zfill(2) #example：01
                if active_spk_id!='None':
                    success = True
        if success:
            clip_face_dir = os.path.join(face_dir, valid_id + '_aligned')
            all_faceFrame_list = [os.path.join(clip_face_dir, i) for i in os.listdir(clip_face_dir)]
            faceFrame_list = []
            for face in all_faceFrame_list:
                if face.split('/')[-1].split('_')[2] == active_spk_id:
                    faceFrame_list.append(face)
        faceFrame_list = sorted(faceFrame_list)
        ...
        if success==False: #无说话人或者说话人对应的人脸过少的情况
            ...
            for para in paraQueue:
                ...
                #--v7 add--
                #如果该段的所有帧全是neutral，才把它作为neutral的候选段
                neutral_list = [] #存放该段中所有选出的neutral片段（其实写在这里最多只能有一个片段（即该段））
                is_all_neutral = True
                for f in para:
                    if f['emo'] != 'neutral':
                        ...
                        break
                if is_all_neutral:
                    frame_list = [i['ID'] for i in para]
                    emoSeg = EmotionSegment(frame_list, 'neutral')
                    neutral_list.append(emoSeg)
                #----------
                ...
                #平滑规则：
                #    1）若 b 是neutral，则 b.length<=4 的情况下将其与前后两个 E 片段合并；
                #    2）若 b 片段是其他情感，则 b.length<=2 的情况下将其与前后两个 E 片段合并
                #------------------------------------------
                ...
                #------------------------------------------
                full_para_frames = [item['ID'] for item in para] #该段中所有帧的列表

                if HF_emotion!=0 and len(HF_emotion_list)>0:
                    #selected_seg = sorted(HF_emotion_list)[-1]
                    selected.append(EmotionSegment(full_para_frames, HF_emotion)) #最终将选出的片段扩展到整个段
                ...
                #窗口的想法：
                #    1、固定窗口的长度为10
                #    2、将窗口从这一段的头滑到尾，如果在某个位置上的count_E >= 4，则将该段视为有效情感E段
                #-----------------------------------
                if HF_emotion!=0 and len(HF_emotion_list)==0:
                    window_len = 10
                    #candiWindow = [] #用于存放候选窗口的列表
                    flag = False #记录是否有有效窗口的变量
                ...
                if len(selected)==0:
                    if len(neutral_list)>0:
                        selected.append(EmotionSegment(full_para_frames, 'neutral'))
...
def load_transcript(transcripts_dir, valid_item):
    ...
    transcript_json = os.path.join(transcripts_dir, movie_name + '.json')
...
def roberta_predict(source_path):
    ...
    roberta_root = '/data8/hzp/RoBERTa'
...
def text_filter(continuous_result_list, text_pos_neg, final_result_list):
    ...
    (去掉这个)elif (roberta预测emotion == neutral)，则该样例有效
    else:
...
if __name__ == '__main__':
    ...
    transcripts_dir = '/data7/emobert/data_nomask_new/transcripts/json' #电影字幕文件的根目录


    #mkdir(pseudo_annotation_dir)

    #情感伪标注
    get_emotion_annotation = AnnotateEmotion(source_root=face_root_dir, audio_root=audio_root_dir, save_root=pseudo_annotation_dir)
    ...
    #-----test----------
    #movie_folder_list = ['No0010.Gone.With.The.Wind']
    movie_folder_list = movie_folder_list[138:]
    #-------------------
    ...
    for movie_name in movie_folder_list:
        ...
        transcript_json = os.path.join(transcripts_dir, movie_name + '.json') #存放该电影字幕信息的json文件
        with open(transcript_json, 'r') as f:
            trans_dict = json.load(f) #{id:{start, end, content}}
        for valid_item in tqdm(valid_list):
            ...
            if continuous_result != None: #denseface伪标注存在
                ...
                current_sentence = trans_dict[valid_id]['content'].lower()
